...
from nlp_core.config.conf_params import get_path_saved_result_file, get_label_by_IID
...
def statistic():
        ...
        if request.form['btn'] == 'btn-upload-file':
            ...
            if filepath != 'undef':
                ...
                with open(PATH_DOWNLOADED, 'w', encoding='utf-8') as f:
                    ...
                    f_writer.writerow(
                        ['post_id', 'content_clean', 'category_dict'])
                    for row in df_raw.itertuples():
                        ...
                        result = {get_label_by_IID(str(index)): prob[index] for index in range(0, len(x_predict)) if
                                  x_predict[index] == 1}
                        f_writer.writerow([row.post_id, x_clean, result])
                    session.pop('_flashes', None)
    ...
    return render_template('statistic.html', title='Statistic',
                           path_file=PATH_STORAGE_FOLDER, path_download=PATH_DOWNLOADED,
...
def download_file(path='/home/trunganh/TrungAnh/CyberspaceCombat/CyberspaceCombat/app/static/output_result/output_predict_test_data.csv'):
    if path is None:
        flash('path is None')
